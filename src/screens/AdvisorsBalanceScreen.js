...
import { fetchAdvisorBalances } from "../services/api";

export default function AdvisorsBalanceScreen({ navigation }) {
  const [searchText, setSearchText] = useState("");
  const [advisorData, setAdvisorData] = useState([]);
  const [loading, setLoading] = useState(true);

  /* =====================================================
        LOAD ADVISOR BALANCES
  ====================================================== */
  useEffect(() => {
    loadAdvisors();
  }, []);
  ...
  const loadAdvisors = async () => {
    try {
      ...
      const result = await fetchAdvisorBalances(userId, token);
      ...
      setAdvisorData(mapped);
    } catch (e) {
      console.log("Advisor list load error:", e);
      Alert.alert("Error", "Failed to load advisor balances.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredData = useMemo(() => {
    if (!searchText.trim()) return advisorData;

    return advisorData.filter((item) =>
      Object.values(item)
        .join(" ")
        .toLowerCase()
        .includes(searchText.toLowerCase())
    );
  }, [searchText, advisorData]);
  ...
  const exportToExcel = async () => {
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, "AdvisorBalances");

      const wbout = XLSX.write(wb, { type: "base64", bookType: "xlsx" });
      const filePath = `${RNFS.DownloadDirectoryPath}/AdvisorBalances.xlsx`;
  ...
  if (loading) {
    return (
      <View style={styles.loadingBox}>
        ...
        <Text style={{ marginTop: 10 }}>Loading Advisor Balances...</Text>
      </View>
    );
  }
    ...
    <View style={styles.container}>
      ...
      <View style={styles.topRow}>
        <Text style={styles.title}>
          Advisor Balances ({filteredData.length})
        </Text>
      ...
      <View style={styles.searchWrapper}>
        ...
        <TextInput
          placeholder="Search advisor or email..."
      ...
      <View style={[styles.row, styles.headerRow]}>
        <Text style={[styles.headerCell, { flex: 1.6 }]}>Advisor#</Text>
        <Text style={[styles.headerCell, { flex: 3 }]}>Advisor</Text>
        <Text style={[styles.headerCell, { flex: 2 }]}>EndDate</Text>
      ...
      <ScrollView>
        {filteredData.map((item, idx) => (
          <View key={idx} style={[styles.row, styles.dataRow]}>
            <Text style={[styles.cell, { flex: 1.6 }]}>{item.id}</Text>
            ...
            <TouchableOpacity
              style={{ flex: 1 }}
              onPress={() =>
                navigation.navigate("AdvisorBalanceDetails", {
                  agentNumber: item.id,
                })
              }
            >
